...
def add_product(product: schemas.Product, db: Session = Depends(get_db)):
    product = db.query(models.Product).filter(models.Product.product_name == product.product_name).first()
    if product:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, 
                            detail="Product already inserted")
    new_product = models.Product(product_name=product.product_name, expiration_date=product.expiration_date, user_id=1)
    db.add(new_product)
    db.commit()
